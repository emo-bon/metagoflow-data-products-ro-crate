...
mandatory_files = [
   "{prefix}_1_clean.fastq.trimmed.fasta",
   "{prefix}_1_clean.fastq.trimmed.qc_summary",
   "{prefix}_2_clean.fastq.trimmed.fasta",
   "{prefix}_2_clean.fastq.trimmed.qc_summary",
   "{prefix}.merged_CDS.faa",
   "{prefix}.merged_CDS.ffn",
   "{prefix}.merged.cmsearch.all.tblout.deoverlapped",
   "{prefix}.merged.fasta",
   "{prefix}.merged.motus.tsv",
   "{prefix}.merged.qc_summary",
   "{prefix}.merged.unfiltered_fasta",
   "fastp.html",
   "final.contigs.fa",
   "RNA-counts"
   ] 
...
"""def joinInterProScanOutputFiles(target_directory, conf):
    ...
    path = os.path.join(target_directory, "functional-annotation", s)
    r = glob.glob(path)
    if len(r) < 2:
    ...
    outfile = os.path.join(target_directory, "functional-annotation",
...
def sequence_categorisation_stanzas(target_directory, template):
    ...
    """
    search = os.path.join(target_directory, "sequence-categorisation", "*.gz")
...
def main(target_directory, yaml_config, debug):
    ...
    for filepath in filepaths:
        ...
        path = os.path.join(target_directory, filepath)
        if not os.path.exists(path):
   ...
   #         "{prefix}.merged_CDS.I5.tsv.gz".format(**conf))
   # ipsf_path = os.path.join(target_directory, ipsf)
   ...
   # filepaths.append(ipsf)
    ...
    url = "https://eosc-fairease-test3.s3.mesocentre.uca.fr/rocrate_template_seqs.txt"
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        for fp in filepaths:
            source = os.path.join(target_directory, fp)
        ...
        ro_crate_name = "%sro-crate" % os.path.split(target_directory)[1]
